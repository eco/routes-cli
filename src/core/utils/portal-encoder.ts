...
export class PortalEncoder {
  ...
  static encodeRoute(route: Intent['route'], chainType: ChainType): Buffer {
    switch (chainType) {
      ...
      case ChainType.TVM:
        return this.encodeTvmRoute(route);
      case ChainType.SVM:
    ...
    const encoded = encodeAbiParameters(
      ...
      [
        [
          route.salt,
          route.deadline,
          AddressNormalizer.denormalize(route.portal, ChainType.EVM) as `0x${string}`,
          route.tokens.map((t) => [
            AddressNormalizer.denormalize(t.token, ChainType.EVM) as `0x${string}`,
            t.amount
          ] as const),
          route.calls.map((c) => [
            AddressNormalizer.denormalize(c.target, ChainType.EVM) as `0x${string}`,
            c.data,
            c.value
          ] as const),
        ],
