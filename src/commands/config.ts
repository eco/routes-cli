...
export function createConfigCommand(): Command {
  ...
  command
    ...
    .action(async (options) => {
      try {
        ...
        logger.error(`Error reading configuration: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  command
    ...
    .action((key, options) => {
      try {
        const config = loadConfig();
        const targetConfig = options.profile 
  ...
  command
    ...
    .action(async (key, options) => {
      try {
        ...
        logger.error(`Error removing configuration: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  profileCommand
    ...
    .action(async (name) => {
      try {
        ...
        logger.error(`Error creating profile: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  profileCommand
    ...
    .action(async (name) => {
      try {
        ...
        logger.error(`Error switching profile: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  profileCommand
    ...
    .action(async (name, options) => {
      try {
        ...
        logger.error(`Error deleting profile: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  profileCommand
    ...
    .action(() => {
      try {
        ...
        logger.error(`Error listing profiles: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
  ...
  command
    ...
    .action(async (options) => {
      try {
        ...
        logger.success(options.profile ? `Profile '${options.profile}' reset` : 'Configuration reset');
      } catch (error) {
        logger.error(`Error resetting configuration: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
...
function displayConfig(config: ConfigSettings): void {
  ...
  if (config.defaultDestinationChain) data['Default Destination Chain'] = config.defaultDestinationChain;
...
async function setConfigInteractive(profileName?: string): Promise<void> {
  ...
  if (answers.defaultDestinationChain) targetConfig.defaultDestinationChain = answers.defaultDestinationChain;
