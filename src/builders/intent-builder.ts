...
export class IntentBuilder {
  ...
  setSalt(salt: Hex): IntentBuilder {
    if (this.intent.route) {
      this.intent.route.salt = salt;
    }
    return this;
  }
  ...
  // Helper method to create from a partial intent
  static fromPartial(partial: Partial<Intent>): IntentBuilder {
    const builder = new IntentBuilder();
    
    if (partial.sourceChainId) builder.setSourceChain(partial.sourceChainId);
    if (partial.destination) builder.setDestinationChain(partial.destination);
    
    if (partial.route) {
      if (partial.route.portal) builder.setPortal(partial.route.portal);
      if (partial.route.deadline) builder.setRouteDeadline(partial.route.deadline);
      if (partial.route.salt) builder.setSalt(partial.route.salt);
      if (partial.route.nativeAmount) builder.addRouteNativeAmount(partial.route.nativeAmount);
      
      partial.route.tokens.forEach(t => builder.addRouteToken(t.token, t.amount));
      partial.route.calls.forEach(c => builder.addCall(c.target, c.data, c.value));
    }
    
    if (partial.reward) {
      if (partial.reward.creator) builder.setCreator(partial.reward.creator);
      if (partial.reward.prover) builder.setProver(partial.reward.prover);
      if (partial.reward.deadline) builder.setRewardDeadline(partial.reward.deadline);
      if (partial.reward.nativeAmount) builder.addRewardNativeAmount(partial.reward.nativeAmount);
      
      partial.reward.tokens.forEach(t => builder.addRewardToken(t.token, t.amount));
    }
